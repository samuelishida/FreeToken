...
def is_triton_kernels_installed() -> bool:
    """True when optional triton-kernels package is importable on CUDA."""
    if is_rocm():
        return False
...
@functools.cache
def is_rocm() -> bool:
    """True when torch is built for ROCm."""
    import torch
...
def driver_hip_version() -> int | None:
    """ROCm driver version, or None when the runtime cannot report it."""
    try:
